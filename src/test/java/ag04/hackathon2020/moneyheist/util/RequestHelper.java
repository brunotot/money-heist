...
public class RequestHelper {
	...
	public MvcResult sendRequest(RequestMethod method, String path, Object body, HttpStatus expectedResponseStatus) throws Exception {
		...
		if (method.equals(RequestMethod.GET)) {
			mvcResult = this.mvc.perform(
				MockMvcRequestBuilders.get(path).contentType(contentType).content(dto))
				.andExpect(resultMatcher).andReturn();
		} else if (method.equals(RequestMethod.DELETE)) {
			mvcResult = this.mvc.perform(
				MockMvcRequestBuilders.delete(path).contentType(contentType).content(dto))
				.andExpect(resultMatcher).andReturn();
		} else if (method.equals(RequestMethod.POST)) {
			mvcResult = this.mvc.perform(
				MockMvcRequestBuilders.post(path).contentType(contentType).content(dto))
				.andExpect(resultMatcher).andReturn();
		} else if (method.equals(RequestMethod.PUT)) {
			mvcResult = this.mvc.perform(
					MockMvcRequestBuilders.put(path).contentType(contentType).content(dto))
					.andExpect(resultMatcher).andReturn();
		} else if (method.equals(RequestMethod.PATCH)) {
			mvcResult = this.mvc.perform(
				MockMvcRequestBuilders.patch(path).contentType(contentType).content(dto))
				.andExpect(resultMatcher).andReturn();
		}
		if (mvcResult == null) {
